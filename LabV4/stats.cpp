...
static void printOne(const char* name, const SearchStats& s) {
    ...
    if (s.found) {
        std::cout << "  Выход: #" << (s.reachedExitIndex + 1) << " (" << s.reachedExit.x << ", " << s.reachedExit.y << ")\n";
        std::cout << "  Длина пути (шаги): " << s.pathLength << "\n";
        std::cout << "  Стоимость пути: " << s.pathCost << "\n";
    }
...
void showCompareScreen(const SearchStats& bfsStats, const SearchStats& dijkstraStats, const SearchStats& astarStats) {
    clearScreen();
...
bool saveCompareReport(
    ...
    auto writeOne = [&](const char* name, const SearchStats& s) {
        ...
        if (s.found) {
            fout << "  Выход: #" << (s.reachedExitIndex + 1) << " (" << s.reachedExit.x << ", " << s.reachedExit.y << ")\n";
            fout << "  Длина пути (шаги): " << s.pathLength << "\n";
            fout << "  Стоимость пути: " << s.pathCost << "\n";
        }
